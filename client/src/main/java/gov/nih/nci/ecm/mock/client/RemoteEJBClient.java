...
import gov.nih.nci.coppa.domain.ClinicalResearchStaff;
...
public class RemoteEJBClient {
    ...
    private static void invokeOrganizationService() throws Exception {
        ...
        System.out.println(org.getIdentifier() + " " + org.getName().getPart().get(0).getValue());
    }
     ...
     * @throws NamingException
     */
    private static void invokePersonService() throws Exception {
        // Let's lookup the remote stateless calculator
        final PersonService prsnSvc = lookupRemotePersonService();
        System.out.println("Obtained a remote stateless PersonService for invocation");

        Ii ii = new Ii();
        ii.setExtension("533689");
        ClinicalResearchStaffDTO staffDto = prsnSvc.getClinicalResearchStaffByPlayerId(ii);
        System.out.println(staffDto.getIdentifier() + " " + staffDto.getPlayerIdentifier());
    }


    /**
     * Looks up and returns the proxy to remote PersonService bean
     *
     * @return
     * @throws NamingException
     */
    private static PersonService lookupRemotePersonService() throws NamingException {
        final Hashtable jndiProperties = new Hashtable();
        ...
        return (PersonService) context.lookup(
                "ejb:ecm-mock/" + MODULE_NM + "/PersonService!" + PersonService.class.getName()
        );
    }
}
